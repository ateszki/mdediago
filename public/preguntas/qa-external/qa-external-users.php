...
*/
		return array(
			'login' => $relative_url_prefix.'../auth/login',
			'register' => $relative_url_prefix.'../auth/register',
			'logout' => $relative_url_prefix.'../auth/logout',
		);
	/*
	...
	//	Until you edit this function, nobody is ever logged in
		...
		if($r["is_loggedin"]){
			return array(
				...
				'email' => $r['email'],
				'level' => ($r['esAdmin'])?QA_USER_LEVEL_SUPER:QA_USER_LEVEL_BASIC
			);
		} 
	/*
		Example 1 - suitable if:
...
*/
	...
	*/
		...
		if (count($publicusernames)) {
			$inQuery = implode(',', array_fill(0, count($publicusernames), '?'));

			$results = DB::select('select name, id FROM users WHERE name IN ('.$inQuery.')', $publicusernames);
			
			foreach ($results as $result){
				$publictouserid[$result->name]=$result->id;
			}
		}
...
*/
	...
	*/
		...
		if (count($userids)) {
			...
			$results = DB::select('select name, id FROM users WHERE id IN ('.$inQuery.')', $userids);
			
			foreach ($results as $result){
				$useridtopublic[$result->id]=$result->name;
			}
		}
...
*/
	...
	*/
		return '<img src="../images/default_user_noimage.png" '.'width="50" height="50" class="qa-avatar-image" alt=""/>';
	}
